...
function renderMaze() {
    ...
    // 第一步：让容器占满可用宽度，获取实际像素宽度
    container.style.width = '100%';
    container.style.height = 'auto';
    container.style.gridTemplateColumns = 'unset';
    container.style.gridTemplateRows = 'unset';

    // 用 requestAnimationFrame 等浏览器完成布局后再精确计算
    requestAnimationFrame(function() {
        ...
        // 实际可用宽度（容器渲染后的精确像素值）
        var playArea = container.parentElement;
        var availW = playArea ? playArea.getBoundingClientRect().width : vw;

        // 计算各区域占用的高度
        var infoH = quizInfo ? quizInfo.getBoundingClientRect().height + 8 : 40;
        var feedbackH = feedback ? feedback.getBoundingClientRect().height + 6 : 30;
        var quizPadV = 30;
        ...
        var maxDim;
        if (isLandscape) {
            var ctrlW = controls ? controls.getBoundingClientRect().width + 12 : 180;
            maxDim = Math.min(availW - ctrlW, availH);
        } else {
            var ctrlH = controls ? controls.getBoundingClientRect().height + 14 : 180;
        ...
        // 渲染格子
        renderMazeCells(container, size, cellSize);
        updateMazeCellHighlight();
    });
}

function renderMazeCells(container, size, cellSize) {
    container.innerHTML = '';
    for (var r = 0; r < size; r++) {
        for (var c = 0; c < size; c++) {
